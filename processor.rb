...
class Processor
  ...
  def self.unique_user_purchases_of_a_type(purchases, type)
    unique_purchases = Set.new
    purchases.each do |purchase|
      next if purchase['type'] != type
      unique_purchases << [purchase['type'], purchase['amount']]
    end
    unique_purchases
  end
  ...
  def parse_user_purchases_file(json_file_name, type)
    ...
    Processor.unique_user_purchases_of_a_type(user_data['purchases'], type.to_s)
  end
end
